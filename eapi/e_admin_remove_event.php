...
try {
    ...
    $result = $eventmedia->eventupdateData_Api(['status' => $data['status']], $table_name, "where id= '$event_id'");
    if ($result) {
        echo json_encode(array("ResponseCode" => "200", "Result" => "true", "ResponseMsg" => "Event has been " . ($data['status'] == '1' ? 'activated' : 'deactivated') . " successfully."));
    }
} catch (Exception $e) {
