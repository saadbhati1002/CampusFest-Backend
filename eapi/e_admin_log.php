...
{
    $username = strip_tags(mysqli_real_escape_string($event,$data['username']));
    $password = strip_tags(mysqli_real_escape_string($event,$data['password']));
    
$chek = $event->query("select * from admin where username='".$username."'  and password='".$password."'");

if($chek->num_rows != 0)
{
    $c = $event->query("select * from admin where username='".$username."'   and password='".$password."'");
    $c = $c->fetch_assoc();
